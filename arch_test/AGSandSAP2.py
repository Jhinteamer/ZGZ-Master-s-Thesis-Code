...
class sa_layer(nn.Module):
    ...
    def __init__(self, channel, groups=64):
        ...
        self.sg = channel // (2 * groups)
        # assert channel // (2 * groups) > 0

        self.cweight = Parameter(torch.zeros(1, self.sg, 1, 1))
        self.cbias = Parameter(torch.ones(1, self.sg, 1, 1))
        self.sweight = Parameter(torch.zeros(1, self.sg, 1, 1))
        self.sbias = Parameter(torch.ones(1, self.sg, 1, 1))

        self.sigmoid = nn.Sigmoid()

        # print('sa1:', self.sg)
        self.gn = nn.GroupNorm(self.sg, self.sg)
...
class sa_layer2(nn.Module):
    ...
    """
    # TODO
    def __init__(self, channel, height, weight, groups=8):
        ...
        assert channel // (2 * groups) > 0
        self.sg = channel // (2 * groups)
        self.fc1 = nn.Conv2d(in_channels=self.sg, out_channels=self.sg, kernel_size=1, bias=False)
        self.fc2 = nn.Conv2d(in_channels=self.sg, out_channels=self.sg, kernel_size=1, bias=False)
        self.relu1 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        xn_avg = self.fc2(self.relu1(self.fc1(xn_avg)))
        xn_max = self.fc2(self.relu1(self.fc1(xn_max)))
        xn = xn_max + xn_avg
        xn = x_0 * self.sigmoid(xn)
...
class UNetWithSAResnet50EncoderandAGsSAP2(nn.Module):
    DEPTH = 6
    ...
    def __init__(self, n_classes=2, input_channels=3, deep_supervision = False, input_h = 224, input_w = 160,**kwargs):
        ...
        self.sa_layer5 = sa_layer2(channel=1024, height=10, weight=14)
        ...
        self.sa_layer4 = sa_layer2(channel=512, height=20, weight=28)
        self.Up_conv4 = conv_block(ch_in=1024, ch_out=512)

        # 512 up 256 256 + 256 = 512 up = 256
        self.Up3 = up_conv(ch_in=512, ch_out=256)
        self.Att3 = Attention_block(F_g=256, F_l=256, F_int=filters[0])
        self.conv3x3_3 = conv3x3(256, 256)
        self.sa_layer3 = sa_layer2(channel=256, height=40, weight=56)
        ...
        self.sa_layer2 = sa_layer2(channel=64, height=80, weight=112, groups=32)
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        for i, block in enumerate(self.down_blocks, 2):
            x = block(x)
            if i == (UNetWithSAResnet50EncoderandAGsSAP2.DEPTH - 1):
                continue
...
def main():
    net = UNetWithSAResnet50EncoderandAGsSAP2(num_classes=2)
    # print("net complete!")
    # print(net)
    # print(net)
