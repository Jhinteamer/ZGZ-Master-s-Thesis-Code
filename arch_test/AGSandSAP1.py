...
class sa_layer2(nn.Module):
    ...
    def __init__(self, channel, height, weight, groups=128):
...
class UNetWithSAResnet50EncoderandAGsSAP1(nn.Module):
    DEPTH = 6
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        for i, block in enumerate(self.down_blocks, 2):
            x = block(x)
            if i == (UNetWithSAResnet50EncoderandAGsSAP1.DEPTH - 1):
                continue
...
def main():
    net = UNetWithSAResnet50EncoderandAGsSAP1(num_classes=2)
    #print(net)
